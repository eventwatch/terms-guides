...
イベントの通報フォームでは、対応結果の返信先として、任意で連絡先（メールアドレス等）を入力できます。この連絡先は、その通報への対応結果をご連絡するためだけに使用します。連絡先を空欄のまま通報することもできますが、その場合は対応結果をご連絡できません。

サービスプロバイダは、**通報者を識別する情報（IP アドレスやそのハッシュ等）を保存しません**。そのため、通報者ご本人からの申し出をもとに特定の通報を探し出すことができず、通報に記載された連絡先について個別の削除依頼にお応えすることができません。

代わりに、**通報への対応が完了または却下されてから 90 日を経過した時点で、連絡先を自動的に消去します**。通報の記録そのもの（対象のイベント・理由区分・記載内容・対応の結果）は、権利者からの削除要請への回答の根拠として残します。
